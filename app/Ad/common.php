...
function api($api,$data,$nosign=''){
    ...
    echo '<pre>';
    print_r($res);
    Config::get('api_debug') && print_r($res);
    $res = json_decode($res,true);
    Config::get('api_debug') && dump($res);
 ...
 */
function authApi($url, $data = array(), $field = null){
    if(!session('?user.openid')){
        return ['code' => 0, 'msg' => '请登录1'];
    }
    $data['openid'] = isset($data['openid']) ? $data['openid'] : session('user.openid');
    return api($url, $data, $field);
}
...
function sign($data,$nosign='',$sign_type=''){
    ...
    ksort($data);
//    dump($data);
    $query=http_build_query($data).'&'.($sign_type == 1 ? $data['sign_code'] : Config::get('api_cfg.sign_code'));
//    dump($query);
